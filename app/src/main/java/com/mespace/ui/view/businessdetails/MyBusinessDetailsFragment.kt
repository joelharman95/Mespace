...
class MyBusinessDetailsFragment : Fragment(), LifecycleObserver {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        PreferenceManager(requireContext()).apply {
            userid = getUserId()
            store_id = getStoreId()
            getStoreDetails(store_id, userid)
        }

        ivSpaceEdit.visibility = View.VISIBLE

        ivSpaceEdit.setOnClickListener {
            findNavController().navigate(R.id.editSpaceFragment)
        }
        if (isChecked) {
